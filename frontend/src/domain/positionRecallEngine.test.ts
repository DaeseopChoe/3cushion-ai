...
describe("positionRecallEngine", () => {
  ...
  it("test2: 시그니처와 무관하게 포지션 일치하면 match", () => {
  ...
  it("test4: L1 거리합 최소가 #1 (동률 시 positionId)", () => {
  ...
  it("test5: 볼별 Manhattan >6 이면 coarse-empty", () => {
    const query: Ball3 = { cue: { x: 10, y: 10 }, target: { x: 50, y: 25 }, second: { x: 40, y: 20 } };
  ...
  it("test6: 후보 4개 중 L1 합 최소 단일 record (Top1)", () => {
    const query: Ball3 = { cue: { x: 10, y: 10 }, target: { x: 50, y: 25 }, second: { x: 40, y: 20 } };
  ...
  it("test7: coarse 통과 시 6D dist2와 무관하게 후보·정렬", () => {
    const query: Ball3 = { cue: { x: 10, y: 10 }, target: { x: 50, y: 25 }, second: { x: 40, y: 20 } };
    const rec = makeRecord(
  ...
  it("test8: v1.0 박스(|dx|≤3∧|dy|≤3) 탈락 → v1.1 볼별 |dx|+|dy|≤6 통과", () => {
    const query: Ball3 = { cue: { x: 10, y: 10 }, target: { x: 50, y: 25 }, second: { x: 40, y: 20 } };
    const rec = makeRecord(

...
describe("canonicalStrategy", () => {
  it("stripRuntimeSysInputs — effective removed, base kept", () => {
  ...
  it("stripRuntimeSysInputs — SSOT set matches EFFECTIVE_KEYS list", () => {
    for (const k of EFFECTIVE_KEYS) {
      assert(
  ...
  it("stripRuntimeSysInputs — null/undefined safe", () => {
  ...
  it("extractBaseSysInputs — system_values wins over lower layers (per-key merge)", () => {
  ...
  it("extractBaseSysInputs — system_values only layer", () => {
    const out = extractBaseSysInputs({
  ...
  it("extractBaseSysInputs — normalizedPayload when no system_values", () => {
    const out = extractBaseSysInputs({
  ...
  it("extractBaseSysInputs — applied.sys.inputs fallback only", () => {
    const out = extractBaseSysInputs({
      appliedSysInputs: {
        CO_f: 30,
        C3_r: 26,
  ...
  it("toCanonicalStrategyEntry — system_values base persist (admin.inputs ignored)", () => {
  ...
  it("toCanonicalStrategyEntry — normalizedPayload fallback when no system_values", () => {
    const draft = toCanonicalStrategyEntry({
      slotId: "S2",
      signature,
  ...
  it("toCanonicalStrategyEntry — applied.sys.inputs fallback", () => {
    const draft = toCanonicalStrategyEntry({
      slotId: "S3",
      signature,
      applied: {
  ...
  it("getPersistableBaseSysInputs — admin.system_values only (Recall hydrate path)", () => {
  ...
  it("getPersistableBaseSysInputs — empty applied, admin.system_values wins", () => {
    const out = getPersistableBaseSysInputs(
  ...
  it("getPersistableBaseSysInputs — effective strip on system_values layer", () => {
    const out = getPersistableBaseSysInputs(
  ...
  it("getPersistableBaseSysInputs — appliedSys.inputs fallback when no system_values", () => {
    const out = getPersistableBaseSysInputs(
      { inputs: { Sn: 99 } },

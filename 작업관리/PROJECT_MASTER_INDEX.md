...
| **ADMIN Initial Target Gate SSOT** | ✅ **COMPLETE** — Fresh ADMIN 진입 시 Target 미지정(`isTargetSelected=false`) 상태에서 Impact Ball 및 Cue→Impact Guide 노출 방지 (`useCoachingController` / `computeCoachingState` ADMIN gate 적용). 명시적 Red/Yellow 더블클릭 시에만 Target 확정 및 Impact/Guide 계산 활성화. (`adminTargetBallRules.contract.test.ts` TEST A~E PASS) |
| **HMR Fresh-State SSOT** | ✅ **COMPLETE** — Fresh/view 초기화 effect에서 `actions.clearAdminWorkSlots()`를 실행하여 HMR/새로고침 후 Fresh ball과 stale slot draft(S1)가 결합되는 Mixed Runtime State 방지. History Recall 정상 스냅샷 hydrate는 완전 보존. (`adminTargetBallRules.contract.test.ts` TEST F~G PASS) |

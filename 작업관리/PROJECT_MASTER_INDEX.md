...
| **AAS Runtime Migration** | **Batch 3 완료 (2026-07-07)** — Application Flow Layer 분리 완료 · App.jsx 6,509 → 5,807 lines (−702) · `application/flows/` 8개 · `domain/lesson/` · `domain/dataset/infra/` · AD-B3-01~05 적용 · D-006/D-007/D-008 Open · **Batch 4 Ready** |
...
**상태:** **Complete** — Application Runtime Constitution (SSOT) 확정 (2026-07-03) · **Runtime Migration In Progress** — Batch 1 완료 (2026-07-06) · **Batch 2 완료 (2026-07-06)** · **Batch 3 완료 (2026-07-07) — Batch 4 Ready**
...
> **주의:** Architecture SSOT 확정 이후 실제 코드 이동은 Batch 단위로 별도 진행한다. **Batch 1 완료 (2026-07-06). Batch 2 완료 (2026-07-06). Batch 3 완료 (2026-07-07). 다음: Batch 4.**
...
| **Batch 4** | 높음 | CAL-002/003/005, MISC-004 | **Ready — Design 착수 가능** |
| Batch 5 | 매우 높음 | TRJ-001/003, RND-003, APP-009 | 미착수 |
| Batch 6 | 최고 | SYS-001/002/003/006, DS-006 | 미착수 (Runtime Contract Blocker) |

### Batch 1 생성 파일

| 파일 | 역할 |
|------|------|
| `frontend/src/domain/system/systemIdentity.ts` | systemId canonicalization · system mode 판정 · useSn 판정 · five-half 판정 · SYS_SYSTEM_CONFIG 임시 은닉 · Batch 6 Runtime Contract 전환 전까지 API Stable / Implementation Replace |
...
- **SYS Overlay**: `components/overlays/SysOverlay.jsx` (AD-B2-01 Pure Presentation, Batch 2 완료). AD-B2-01: `computeValues`/`solveFiveHalf` props 주입. `admin/sys/SysOverlay.tsx`는 메인 트리 미사용.
...
| Renderer Trajectory | `renderer/trajectory/trajectoryRenderModel.ts` (`buildTrajectoryRenderModel`), `renderer/trajectory/anchorConversionModel.ts` (`buildRgAnchors`) |
...
- **AAS Runtime Migration Batch 3** — Application Flow · Search · Dataset · AI Domain 분리 완료 (2026-07-07):
  ...
  - ✔ Migration Debt D-006/D-007/D-008 Open 등록 · Batch 3 Baseline 확정
...
- **AAS Runtime Migration Batch 4** — CAL-002/003/005, MISC-004 — 고위험 계산 Domain 분리 · **Design 착수 가능 (Batch 4 Ready)**
...
> **Architecture 상태:** Application Architecture Standard (AAS) v2.0 **완료**. **AAS Runtime Migration 진행 중 — Batch 1 완료 (2026-07-06) · Batch 2 완료 (2026-07-06) · Batch 3 완료 (2026-07-07).** 다음 우선순위는 **Batch 4 Design**.

### 최우선 — AAS Runtime Migration Batch 4

- Batch 3 완료 (2026-07-07). **Batch 4 Design 착수 가능.**
- 대상: CAL-002/003/005, MISC-004 — 고위험 계산 Domain 분리
- Batch 3 Baseline: `b7d7712` (STEP 3-8 ballDragFlow extraction)
- 실제 코드 이동은 `App_Migration_Map.md` Batch 순서(1→6)와 Part D Review Checklist를 기준으로 수행한다.

### P0 — AAS Runtime Migration Batch 4 (Design 착수)
